...
export function sanitizeJudgeOutput(raw: unknown, aCount: number, bCount: number): JudgeOutput {
  ...
  const closesCycle = (from: string, to: string): boolean => {
    for (
      ...
      current !== undefined;
      current = duplicateLinks.get(current)
    ) {
      if (current === from) {
        return true
      }
  ...
  for (const item of Array.isArray(r.decisions) ? r.decisions : []) {
    ...
    if (!validCandidateId(d.id, aCount, bCount) || seen.has(d.id)) {
      continue
    }
    if (d.action !== 'keep' && d.action !== 'reject') {
      continue
    }
    seen.add(d.id)
    const target =
      validCandidateId(d.duplicate_of, aCount, bCount) && d.duplicate_of !== d.id
        ? d.duplicate_of
        : undefined
    const duplicateOf = target !== undefined && !closesCycle(d.id, target) ? target : undefined
    if (duplicateOf !== undefined) {
      duplicateLinks.set(d.id, duplicateOf)
    }
    const reason =
      typeof d.reason === 'string'
        ? d.reason.trim().slice(0, JUDGE_REASON_MAX) || undefined
        : undefined
    const severity = SEVERITIES.has(d.severity as FindingSeverity)
      ? (d.severity as FindingSeverity)
      : undefined
    decisions.push({
      id: d.id,
      action: d.action,
...
export function dedupeExactCrossLane(a: SanitizedReview, b: SanitizedReview): CrossLaneDedup {
  ...
  a.findings.forEach((finding, index) => {
    ...
    if (key !== null && !aIndexByKey.has(key)) {
      aIndexByKey.set(key, index)
    }
  })
  ...
  b.findings.forEach((finding, oldIndex) => {
    ...
    const severity =
      SEVERITY_ORDER[finding.severity] > SEVERITY_ORDER[kept.severity]
        ? finding.severity
        : kept.severity
  ...
  return {
    a: { ...a, findings: aFindings },
    b: { ...b, findings: bFindings, narrative: bNarrative },
    merged,
  }
}
 ...
 */
export function assembleDualReview(
  a: SanitizedReview,
  b: SanitizedReview,
  judge: JudgeOutput,
): DualAssembly {
  ...
  const rootOf = (id: string): string => {
    ...
    for (;;) {
      ...
      if (!next || !candidateById.has(next) || visited.has(next)) {
        return current
      }
      visited.add(next)
      current = next
  ...
  for (const [rootId, members] of membersByRoot) {
    ...
    if (rejected) {
      representative = securityMember ?? keptMember ?? representative
    }
    const group: Group = {
      representative,
      members,
    ...
    for (const member of members) {
      groupByMemberId.set(member.id, group)
    }
  }
  ...
  for (const group of groups) {
    if (!group.survives) {
      stats.rejected += group.members.length
    } else {
      stats.merged += group.members.length - 1
    }
  }
  stats.added_by_b = groups.filter(
    (g) => g.survives && g.representative.side === 'B' && !g.consensus,
  ).length

  const mergedFinding = (group: Group): Finding => {
    const severities = group.members.map((m) => m.finding.severity)
    const maxSeverity = severities.reduce((worst, s) =>
  ...
  for (const candidate of candidates) {
    ...
    if (!group.survives || group.representative.id !== candidate.id) {
      continue
    }
  ...
  for (const candidate of candidates) {
    ...
    if (finalIndex !== undefined) {
      newIndexByAIndex.set(candidate.index, finalIndex)
    }
  }
  ...
  for (const candidate of candidates) {
    if ((groupByMemberId.get(candidate.id) as Group).survives) {
      laneSurvived[candidate.side] = true
    }
  }
  const verdicts: Verdict[] = []
  if (a.findings.length === 0 || laneSurvived.A) {
    verdicts.push(a.verdict)
  }
  if (b.findings.length === 0 || laneSurvived.B) {
    verdicts.push(b.verdict)
  }
